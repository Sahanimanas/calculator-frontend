// components/Project/EditClientModal.jsx
import React, { useEffect, useState } from "react";
import { toast } from "react-hot-toast";
import axios from "axios";

const EditClientModal = ({ isOpen, onClose, client, geography, refreshClients }) => {
  ...
  useEffect(() => {
    if (client) {
      ...
      setGeographyId(client.geography_id || geography?._id || "");
    }
  }, [client, geography]);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.put(`${apiUrl}/client/${client._id}`, {
        name: name.trim(),
      ...
      <div className="bg-white rounded-lg shadow-lg w-full max-w-lg p-6 relative">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold">
            Edit <span className="text-purple-700">Client</span>
          </h2>
          <button
            onClick={onClose}
