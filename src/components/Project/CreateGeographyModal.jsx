// components/Project/CreateGeographyModal.jsx
import React, { useState } from "react";
import { toast } from "react-hot-toast";
import axios from "axios";

const CreateGeographyModal = ({ isOpen, onClose, refreshGeographies }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${apiUrl}/geography`, {
        name: name.trim(),
      ...
      if (error.response?.status === 409) {
        ...
        toast.error(error.response?.data?.error || "Failed to create geography");
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="bg-white rounded-lg shadow-lg w-full max-w-lg p-6 relative">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold">
            Create New <span className="text-blue-700">Geography</span>
          </h2>
          <button
            onClick={onClose}
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end gap-2 mt-6">
            ...
            >
              {loading ? "Creating..." : "Create Geography"}
            </button>
          </div>
        </form>
